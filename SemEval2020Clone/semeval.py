        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args['gradient_accumulation_steps'] == 0:
                ...
                if args['logging_steps'] > 0 and global_step % args['logging_steps'] == 0:
                    ...
                    print(f"Logging steps: {global_step}")
...
def get_eval_report(labels, preds, dataset, processor):
...
def compute_metrics(task_name, preds, labels, dataset, processor):
    assert len(preds) == len(labels)
    print("******* Computing metrics for "+task_name+" ********")
    return get_eval_report(labels, preds, dataset, processor)
...
def evaluate(model, tokenizer, prefix="", dataset=True):
    ...
    result, wrong = compute_metrics(EVAL_TASK, preds, out_label_ids, dataset, processor=processors[EVAL_TASK]())
